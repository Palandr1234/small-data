...
object MovieLensALS {
  ...
  def main(args: Array[String]) {
    ...
    if (doGrading){
      ...
      val ids = sc
        .textFile(ratingsPath + "/user_ratings.tsv")
        .map(x => x.split("\t"))
        .map(x => x(0).toInt)
        .collect()
        .toSet
  ...
  def loadRatings(path: String, sc: SparkContext) = {
    ...
    val ratingsDataBefore = sc.textFile(path).map { line =>
      val cols = line.split(",")
      (cols(1).toInt, Rating(cols(0).toInt, cols(1).toInt, cols(2).toDouble))
    }
    
    val filter = ratingsDataBefore
      .map(x => (x._1, 1))
      .reduceByKey(_+_)
      .filter(x => x._2 >= 75)

    val ratingsData = ratingsDataBefore
       .join(filter)
       .map(x => x._2._1)
  ...
  def rmse(test: RDD[Rating], prediction: scala.collection.Map[Int, Double]) = {
    val rating_pairs = test.map( x =>
    	(x.product, (prediction.getOrElse(x.product, 0.0), x.rating))
    )
