...
class Grader(path: String, sc: SparkContext) {
  ...
  println("Do you want to load grades from file? y/[n]")
  var loadGrades = scala.io.StdIn.readLine()

  if (loadGrades == "y" || loadGrades == "yes" || loadGrades == "Yes" || loadGrades == "YES") {
    graded = sc
      .textFile(path + "/user_ratings.tsv")
      .map(x => x.split("\t"))
